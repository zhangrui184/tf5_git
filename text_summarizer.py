...
class MiniConfig(object):
    ...
    learning_rate_decay_factor = 0.99
    max_gradient_norm = 5.0
    num_samples = 64  # Sampled Softmax
    batch_size = 10
    size = 5  # Number of Node of each layer
    num_layers = 2
    vocab_size = 10000
#config = LargeConfig()
config=MiniConfig()
...
def train():
  ...
  #  src_train, dest_train, src_dev, dest_dev= data_util.prepare_headline_data( FLAGS.data_dir, FLAGS.vocab_size)
        #data_dir=FLAGS.data_dir, vocabulary_size=FLAGS.vocab_size, tokenizer=data_util.jieba_tokenizer)

    src_train='D:\python project me\data/text_summar/train\content_train_id.txt'
    dest_train='D:\python project me\data/text_summar/train/title_train_id.txt'
    src_dev='D:\python project me\data/text_summar\dev\content_dev_id.txt'
    dest_dev='D:\python project me\data/text_summar\dev/title_dev_id.txt'
   # src_test=data_util.src_test_path
  #  dest_test=data_util.dest_test_path
    # device config
   ...
   # with tf.Session() as sess:
        ...
        while True:
            ...
            bucket_id = min([i for i in range(len(trainbuckets_scale))
